...
Glancing at the glimpse and summary of the data, I can see there are multiple NAs in the code.
...
There are 32 herbivores in the data. The list of herbivores is below. 
...
large_mammals <- data.frame(large_weight)
large_mammals
```
...
```r
small_weight <- filter(sleep, bodywt <= 19)
small_mammals <- data.frame(small_weight)
small_mammals
```
...
Small animals sleep longer on average, averaging around 11.786 hours compared to 3.3 for large animals. 
...
The sleepiest animal is the little brown bat with a total of 19.9 hours. 
