...
```
The NAs in the amniota set are represented by the value -999 instead of NA. The NAs in the amphibio set are represented by NA. 
...
```
NAs in mammalia makes sense since mammals don't usually lay eggs (with the exception of the platypus and echidna). NAs in aves or reptilia could mean it is hard to measure the mass of their eggs (i.e. hard to actually weigh them, difficult to obtain, etc.). These results affect my interpretation because NA could mean non-applicable. 
...
amphibio_species_class <- amphibio_new %>%
  ...
  summarize_all(~(sum(is.na(.))))
```
I think the NAs would be for species that overlap in classification (for example, they could be fossorial and terrestrial). The authors could have also included NAs if there was no data available. 
